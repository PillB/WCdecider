  ...
  <main class="mx-auto max-w-7xl px-4 py-8">
    <section class="rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-950 p-6 md:p-8 mb-8">
      <div class="grid lg:grid-cols-[1.25fr_.75fr] gap-8 items-center">
        ...
        <svg viewBox="0 0 520 360" role="img" aria-labelledby="flow-title flow-desc" class="w-full">
          <title id="flow-title"><tspan class="en">WCdecider model workflow</tspan><tspan class="es">Flujo del modelo WCdecider</tspan></title>
          <desc id="flow-desc"><tspan class="en">Sources flow through chronological model selection, untouched holdout validation, market comparison, audit, JSON, and report generation.</tspan><tspan class="es">Las fuentes pasan por selección cronológica, validación holdout intacta, comparación de mercado, auditoría, JSON y generación del informe.</tspan></desc>
          ...
          <g fill="#e2e8f0" font-family="system-ui" font-size="14" text-anchor="middle">
            ...
            <text x="92" y="140"><tspan class="en">Elapsed WC</tspan><tspan class="es">Mundial jugado</tspan></text><text x="92" y="160"><tspan class="en">40 results</tspan><tspan class="es">40 resultados</tspan></text>
            <text x="92" y="228"><tspan class="en">Screenshots</tspan><tspan class="es">Capturas</tspan></text><text x="92" y="248">Betano + Betsson</text>
            <text x="277" y="106">Elo 1X2</text><text x="277" y="130"><tspan class="en">walk-forward selection</tspan><tspan class="es">selección walk-forward</tspan></text>
            <text x="277" y="154"><tspan class="en">untouched holdout</tspan><tspan class="es">holdout intacto</tspan></text><text x="277" y="178"><tspan class="en">calibration + stress</tspan><tspan class="es">calibración + estrés</tspan></text>
            <text x="445" y="52"><tspan class="en">Probabilities</tspan><tspan class="es">Probabilidades</tspan></text><text x="445" y="72">1X2</text>
            <text x="445" y="140">PASS / HALT</text><text x="445" y="160"><tspan class="en">no actionable bets</tspan><tspan class="es">sin apuestas accionables</tspan></text>
            <text x="445" y="228">JSON + audit</text><text x="445" y="248"><tspan class="en">32 synced cards</tspan><tspan class="es">32 tarjetas sincronizadas</tspan></text>
          </g>
    ...
    <section class="mb-8 grid lg:grid-cols-[1fr_1fr] gap-5">
      <div class="rounded-3xl border border-slate-800 bg-slate-900 p-5">
        ...
        <ol class="mt-3 space-y-2 text-sm text-slate-300 list-decimal pl-5">
          <li><span class="en">There are zero actionable bets in this release.</span><span class="es">No hay apuestas accionables en esta versión.</span></li>
          <li><span class="en">PASS means the betting policy is not validated; HALT means the apparent edge is suspicious and needs investigation.</span><span class="es">PASS significa que la política de apuestas no está validada; HALT significa que la ventaja aparente es sospechosa y necesita investigación.</span></li>
      ...
      <div class="rounded-3xl border border-slate-800 bg-slate-900 p-5">
        ...
        <div class="mt-3 text-sm text-slate-300 space-y-2">
          ...
          <p><span class="tip" tabindex="0">PASS<span><span class="en">The comparison is published for transparency, but no profitable betting policy has been validated.</span><span class="es">La comparación se publica por transparencia, pero no se ha validado una política de apuestas rentable.</span></span></span>: <span class="en">abstain.</span><span class="es">abstenerse.</span></p>
...
let DATA=null,METRICS=null,AUDIT_COUNT=0;
...
function renderSummary(rows){
 ...
 $('#summary').innerHTML=metric('<span class="en">Fixtures</span><span class="es">Partidos</span>',rows.length)+metric('<span class="en">Actionable bets</span><span class="es">Apuestas accionables</span>',0)+metric('HALT',halted)+metric('<span class="en">Holdout Brier</span><span class="es">Brier holdout</span>',Number(METRICS.calibration.holdout_brier).toFixed(4));
 ...
 <p class="mt-2 text-amber-200"><span class="en">This validates forecast signal only. Historical closing odds are incomplete, so profitability, CLV, ROI and staking tiers are not validated.</span><span class="es">Esto valida solo señal predictiva. Las cuotas históricas de cierre están incompletas, por lo que rentabilidad, CLV, ROI y niveles de apuesta no están validados.</span></p>`;
}
 ...
 return `<article class="card-anchor bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden" data-fixture-id="${escapeHtml(row.fixture_id)}" data-date="${row.kickoff_lima.slice(0,10)}" data-strength="${strength}">
   ...
   <div class="p-5">
    <div class="grid grid-cols-3 gap-2 text-center">
     <div data-json-pointer="${ptr}/probabilities/team_a_win">${metric('1',`${(row.probabilities.team_a_win*100).toFixed(1)}%`)}</div>
     <div data-json-pointer="${ptr}/probabilities/draw">${metric('X',`${(row.probabilities.draw*100).toFixed(1)}%`)}</div>
     <div data-json-pointer="${ptr}/probabilities/team_b_win">${metric('2',`${(row.probabilities.team_b_win*100).toFixed(1)}%`)}</div>
    </div>
    ...
    <div class="mt-4 rounded-2xl border border-slate-700 bg-slate-950 p-4">
      <div class="text-xs text-slate-500"><span class="en">Highest-EV screenshot comparison — not actionable</span><span class="es">Comparación de captura con mayor EV — no accionable</span></div>
      <div class="mt-1 font-bold text-white" data-json-pointer="${ptr}/recommendation/selection_original">${rec}</div>
      ${r?`<div class="mt-2 grid grid-cols-3 gap-2 text-sm"><div data-json-pointer="${ptr}/recommendation/ev_pct">EV <b>${r.ev_pct.toFixed(1)}%</b></div><div data-json-pointer="${ptr}/recommendation/stressed_ev_pct"><span class="en">Stress</span><span class="es">Estrés</span> <b>${r.stressed_ev_pct.toFixed(1)}%</b></div><div data-json-pointer="${ptr}/recommendation/confidence"><span class="en">Confidence</span><span class="es">Confianza</span> <b>${r.confidence}%</b></div></div>
    ...
    <details class="mt-4 rounded-2xl border border-slate-800 p-4">
      ...
      <div class="mt-3 space-y-3 text-sm text-slate-300">
        ...
        <div class="en"><ul class="list-disc pl-5">${row.risk_notes.en.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul></div>
        <div class="es"><ul class="list-disc pl-5">${row.risk_notes.es.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul></div>
        <p><b>ELI5:</b> <span class="en">This row compares a model probability with an old screenshot price. PASS and HALT both mean “do not treat as a bet.” Use it to inspect the model, not to execute a wager.</span><span class="es">Esta fila compara una probabilidad del modelo con una cuota antigua de captura. PASS y HALT significan “no tratar como apuesta”. Úsala para inspeccionar el modelo, no para ejecutar una apuesta.</span></p>
...
async function load(){
 try{
  ...
  const auditRows=auditText.trim().split(/\r?\n/).slice(1);
  if(!auditRows.length||auditRows.some(line=>!line.endsWith(',PASS')))throw new Error('Datapoint audit is incomplete or blocked');
 ...
 $('#status').innerHTML='<span class="en">Verified JSON loaded</span><span class="es">JSON verificado cargado</span>';
  ...
  $('#error').classList.remove('hidden'); $('#error').innerHTML=`<span class="en">Report data failed to load: ${escapeHtml(error.message)}</span><span class="es">No se pudieron cargar los datos del informe: ${escapeHtml(error.message)}</span>`;
  $('#status').innerHTML='<span class="en">DATA ERROR</span><span class="es">ERROR DE DATOS</span>';
 }
}
