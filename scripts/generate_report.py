 ...
 return `<article class="card-anchor bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden" data-fixture-id="${escapeHtml(row.fixture_id)}" data-date="${row.kickoff_lima.slice(0,10)}" data-strength="${strength}">
   ...
   <div class="p-5">
    ...
    <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2 text-center">
      ...
      <div data-json-pointer="${ptr}/common_markets/totals/2/over_probability">${metric('<span class="en">Over 2.5</span><span class="es">Más de 2,5</span>',`${pct(over25.over_probability)} · ${odds(over25.over_fair_odds)}`,help.over_2_5)}</div>
      <div data-json-pointer="${ptr}/common_markets/btts/yes_probability">${metric('BTTS Yes / Sí',`${pct(row.common_markets.btts.yes_probability)} · ${odds(row.common_markets.btts.yes_fair_odds)}`,help.btts_yes)}</div>
    </div>
    <div class="mt-3 rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-sm">
      ...
      <div class="mt-2 grid sm:grid-cols-3 gap-2 text-center">
        ${metric('<span class="en">Under 2.5</span><span class="es">Menos de 2,5</span>',`${pct(over25.under_probability)} · ${odds(over25.under_fair_odds)}`,help.under_2_5)}
        ${metric('BTTS No',`${pct(row.common_markets.btts.no_probability)} · ${odds(row.common_markets.btts.no_fair_odds)}`,help.btts_no)}
        ${metric('<span class="en">Home -0.5 AH</span><span class="es">Local -0,5 HA</span>',`${pct(ah.home_probability)} · ${odds(ah.home_fair_odds)}`,help.home_minus_0_5)}
      </div>
