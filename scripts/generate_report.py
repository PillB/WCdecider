      ...
      <div class="grid lg:grid-cols-[1.25fr_.75fr] gap-8 items-center">
        ...
        <svg viewBox="0 0 520 360" role="img" aria-labelledby="flow-title flow-desc" class="w-full">
          ...
          <g fill="#e2e8f0" font-family="system-ui" font-size="14" text-anchor="middle">
            ...
            <text x="277" y="106"><tspan class="en">Elo + score model</tspan><tspan class="es">Elo + modelo de goles</tspan></text><text x="277" y="130"><tspan class="en">walk-forward tuning</tspan><tspan class="es">ajuste walk-forward</tspan></text>
            <text x="277" y="154"><tspan class="en">untouched holdout</tspan><tspan class="es">holdout intacto</tspan></text><text x="277" y="178"><tspan class="en">Poisson + DC shadow</tspan><tspan class="es">Poisson + DC sombra</tspan></text>
            <text x="445" y="52"><tspan class="en">Probabilities</tspan><tspan class="es">Probabilidades</tspan></text><text x="445" y="72"><tspan class="en">1X2 + goals + AH</tspan><tspan class="es">1X2 + goles + AH</tspan></text>
            <text x="445" y="140">BEST AVAILABLE</text><text x="445" y="160"><tspan class="en">risk-ranked pick</tspan><tspan class="es">opción por riesgo</tspan></text>
    ...
    <section class="mb-8 grid lg:grid-cols-[1fr_1fr] gap-5">
      <div class="rounded-3xl border border-slate-800 bg-slate-900 p-5">
        ...
        <ol class="mt-3 space-y-2 text-sm text-slate-300 list-decimal pl-5">
          <li><span class="en">Every match has one best-available recommendation; none is a guarantee.</span><span class="es">Cada partido tiene una recomendación de mejor opción disponible; ninguna es garantía.</span></li>
...
function renderSummary(rows){
 ...
 <p class="mt-2 text-amber-200"><span class="en">The report ranks one best-available pick per match. Historical closing odds are absent for expanded families, so profitability, CLV, ROI and staking tiers remain unvalidated.</span><span class="es">El informe ordena una mejor opción disponible por partido. No hay cuotas históricas de cierre para mercados ampliados, por lo que rentabilidad, CLV, ROI y niveles de apuesta siguen sin validarse.</span></p>`;
}
...
function card(row,index){
 ...
 const over25=row.common_markets.totals.find(x=>x.line===2.5);
 const ah=row.common_markets.asian_handicap.find(x=>x.home_line===-0.5);
 const comparisons=row.market_comparisons.slice(0,8).map((m,i)=>`<tr class="border-t border-slate-800">
 ...
 return `<article class="card-anchor bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden" data-fixture-id="${escapeHtml(row.fixture_id)}" data-date="${row.kickoff_lima.slice(0,10)}" data-strength="${strength}">
   ...
   <div class="p-5">
    ...
    <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2 text-center">
      <div data-json-pointer="${ptr}/expected_goals/team_a">${metric('<span class="en">Expected goals 1</span><span class="es">Goles esperados 1</span>',Number(row.expected_goals.team_a).toFixed(2))}</div>
      <div data-json-pointer="${ptr}/expected_goals/team_b">${metric('<span class="en">Expected goals 2</span><span class="es">Goles esperados 2</span>',Number(row.expected_goals.team_b).toFixed(2))}</div>
      <div data-json-pointer="${ptr}/common_markets/totals/2/over_probability">${metric('<span class="en">Over 2.5</span><span class="es">Más de 2,5</span>',`${pct(over25.over_probability)} · ${odds(over25.over_fair_odds)}`)}</div>
      <div data-json-pointer="${ptr}/common_markets/btts/yes_probability">${metric('BTTS Yes / Sí',`${pct(row.common_markets.btts.yes_probability)} · ${odds(row.common_markets.btts.yes_fair_odds)}`)}</div>
    </div>
    <div class="mt-3 rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-sm">
      ...
      <div class="mt-2 grid sm:grid-cols-3 gap-2 text-slate-300">
        <div><span class="en">Under 2.5</span><span class="es">Menos de 2,5</span>: <b>${pct(over25.under_probability)}</b> · ${odds(over25.under_fair_odds)}</div>
        <div>BTTS No: <b>${pct(row.common_markets.btts.no_probability)}</b> · ${odds(row.common_markets.btts.no_fair_odds)}</div>
        <div><span class="en">Home -0.5 AH</span><span class="es">Local -0,5 HA</span>: <b>${pct(ah.home_probability)}</b> · ${odds(ah.home_fair_odds)}</div>
      </div>
