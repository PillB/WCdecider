...
function renderDiagnostics(error){
 ...
 el.innerHTML=`<h2 class="font-bold text-white"><span class="en">Load diagnostics</span><span class="es">Diagnóstico de carga</span></h2>
  ...
  <p>Online: ${navigator.onLine?'yes':'no'} · Build: ${escapeHtml(build.slice(0,12))} · Audit: ${escapeHtml(audit.slice(0,12))}</p>
  <p>User agent: ${escapeHtml(navigator.userAgent)}</p>`;
}
   ...
   <div class="p-5">
    ...
    <section class="research-panel hidden mt-4 rounded-2xl border border-violet-800 bg-violet-950/30 p-4 text-sm" data-research-candidate="${escapeHtml(research.selected_candidate||'')}">
      ...
      <div class="mt-3 grid grid-cols-2 md:grid-cols-5 gap-2 text-center">
        ...
        ${metric('BTTS Yes',`${safePct(researchBtts.yes_probability)} (${Number(research.deltas_vs_production?.btts_yes_pp||0).toFixed(2)}pp)`)}
      </div>
...
async function load(){
 try{
  ...
  ['#date-filter','#strength-filter','#research-toggle'].forEach(selector=>{const el=$(selector);if(el)el.disabled=false;});
  const loading=$('#loading-shell'); if(loading)loading.classList.add('hidden');
  APP_READY=true;
