...
function card(row,index){
 ...
 return `<article class="card-anchor bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden" data-fixture-id="${escapeHtml(row.fixture_id)}" data-date="${row.kickoff_lima.slice(0,10)}" data-strength="${strength}">
   ...
   <div class="p-5">
    ...
    <section class="mt-4 rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
    ...
    <section class="research-panel hidden mt-4 rounded-2xl border border-violet-800 bg-violet-950/30 p-4 text-sm" data-research-candidate="${escapeHtml(research.selected_candidate||'')}">
    ...
    <div class="mt-4 rounded-2xl border border-slate-700 bg-slate-950 p-4">
      <div class="text-xs text-red-300"><span class="en">ABSTAIN — highest-ranked comparison only</span><span class="es">ABSTAIN — solo comparación mejor clasificada</span></div>
      <div class="mt-1 font-bold text-white" data-json-pointer="${ptr}/rank_one_comparison/selection_original">${rec}</div>
