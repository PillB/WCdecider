    ...
    <section class="mb-8 grid lg:grid-cols-[1fr_1fr] gap-5">
      <div class="rounded-3xl border border-slate-800 bg-slate-900 p-5">
        ...
        <ol class="mt-3 space-y-2 text-sm text-slate-300 list-decimal pl-5">
          <li><span class="en">Every match may show ranked sourced comparisons, but the current release abstains from every bet and allocates zero stake.</span><span class="es">Cada partido puede mostrar comparaciones con fuente, pero la versión actual se abstiene de todas las apuestas y asigna monto cero.</span></li>
    ...
    <section class="mt-10 grid lg:grid-cols-2 gap-5">
      <div class="rounded-3xl border border-slate-800 bg-slate-900 p-5">
        ...
        <ol class="mt-3 list-decimal pl-5 space-y-2 text-sm text-slate-300">
          ...
          <li><span class="en">Confirm full-time settlement. The current release authorizes no bet and assigns zero stake.</span><span class="es">Confirma que liquida al tiempo reglamentario. La versión actual no autoriza apuestas y asigna monto cero.</span></li>
        </ol>
      </div>
      <div class="rounded-3xl border border-slate-800 bg-slate-900 p-5">
...
function renderSummary(rows){
 ...
 <p class="mt-2 text-red-200"><span class="en">Release blocked: ranked rows are audit comparisons only. All decisions are ABSTAIN and all stakes are zero until clean data, fresh prices, a sealed holdout, and profitability validation exist.</span><span class="es">Versión bloqueada: las filas ordenadas son solo comparaciones de auditoría. Todas las decisiones son ABSTAIN y todos los montos son cero hasta contar con datos limpios, cuotas vigentes, holdout sellado y validación de rentabilidad.</span></p>`;
}
...
async function load(){
 try{
  ...
  ['#date-filter','#strength-filter','#risk-profile','#research-toggle'].forEach(selector=>{const el=$(selector);if(el)el.disabled=false;});
  applyRiskProfile();
  const loading=$('#loading-shell'); if(loading)loading.classList.add('hidden');
  APP_READY=true;
...
function boot(){
 ...
 const date=$('#date-filter'),strength=$('#strength-filter'),riskProfile=$('#risk-profile');
 if(date)date.addEventListener('change',applyFilters); if(strength)strength.addEventListener('change',applyFilters); if(riskProfile)riskProfile.addEventListener('change',applyRiskProfile);
 bindTips();
