...
def test_static_report_contains_bilingual_contract():
    ...
    assert "32 matches. One auditable pipeline." in html
    assert "32 partidos. Un pipeline auditable." in html
...
def test_toggle_changes_visible_language():
    ...
    with serve(SITE) as url, sync_playwright() as p:
        ...
        assert page.locator("text=32 matches. One auditable pipeline.").is_visible()
        page.locator("#lang").click()
        assert "lang-es" in page.locator("body").get_attribute("class")
        assert page.locator("text=32 partidos. Un pipeline auditable.").is_visible()
